...
//				System.out.println("no");
	...
	static boolean isItSafe(int maze[][], int x, int y, int n) {
	...
	static boolean ratIn_Maze(int[] maze[], int x, int y, int n, int solMaze[][]) {

		if (x == n - 1 && y == n - 1) {
			solMaze[x][y] = 1;
			return true;
		}
		if (isItSafe(maze, x, y, n)) {
			solMaze[x][y] = 1;

			if (ratIn_Maze(maze, x + 1, y, n, solMaze)) {
				return true;
			}
			if (ratIn_Maze(maze, x, y + 1, n, solMaze)) {
				return true;
			}
